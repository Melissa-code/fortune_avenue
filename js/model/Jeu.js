...
class Jeu {
  constructor() {
    ...
    this.piocheChance =
      CarteEffetsFactory.chargerDataEffetsCartes(effetsChanceJson);
    CarteEffetsFactory.melangerCartes(this.piocheChance);

    this.piocheFondsCommun = 
      CarteEffetsFactory.chargerDataEffetsCartes(effetsFondsCommunsJson);
  ...
  payerLoyer(joueurCourant, caseJeu) {
    ...
    if (montantLoyer > 0) {
      ...
      messages.push(
        `${joueurCourant.nom} paie ${montantLoyer} M de loyer à ${proprietaire.nom} pour la propriété "${caseJeu.nom}".`,
      );
    }
    return messages;
  }
  ...
  filtrerPropositionsValablesFondsCommuns(_joueur) {
  ...
  soumettreProposition(numProposition) {
    ...
    const success = this.listePropositions[numProp].valider(
      joueurCourant,
      this,
      this.casesJeu[joueurCourant.position],
      this.banque,
    );
    if (!success) return;
