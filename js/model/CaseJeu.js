...
import { Proposition, PropositionDecliner } from "./Proposition.js";
import EtatsJeu from "./enums/EtatsJeu.js";

/**
 * classe mère Case de jeu
 *  nom : nom de la case
 *  arriver(joueur, jeu) : méthode appelée quand un joueur arrive sur la case
 */
...
/**
 * proprietes : rues, gares, societes
 */
export class CasePropriete extends CaseJeu {
  ...
  /**
   * proposer au joueur d'acheter la propriete
   * s'il n'y a pas de proprietaire, sinon payer le loyer
   */
  filtrerPropositionsValables(joueur, jeu) {
    ...
    if (
      propositionsValables.length === 1 &&
      propositionsValables[0] instanceof PropositionDecliner
    ) {
      return [];
    }
  ...
  // TODO: hypotheque et leverHypotheque à implémenter
  // hypothequer() {
...
export class CaseRue extends CasePropriete {
  ...
  construire(typeConstruction, banque) {
    if (typeConstruction === "maison") {
      ...
      console.log(`type de construction inconnu: ${typeConstruction}`);
    }
  }
...
export class CaseGare extends CasePropriete {
  ...
  calculerLoyer(_jeu = null) {
    ...
    const nbGares = joueurProprietes.filter(
      (propriete) => propriete instanceof CaseGare,
    ).length;
...
export class CaseSociete extends CasePropriete {
  ...
  calculerLoyer(jeu) {
    ...
    let montant;
    const joueurProprietes = this.proprietaire.proprietes || [];
    const nbSocietes = joueurProprietes.filter(
      (propriete) => propriete instanceof CaseSociete,
    ).length;
