...
export class CarteEffetsFactory {
  /**
   * parcourt les données de data/effets_chance.js/data/effets_fonds_communs.js
  ...
  static generateCarte(jsonObj) {
    switch (jsonObj.type) {
      ...
      default:
        console.log(`Type de carte chance inconnu: ${jsonObj.type}`);
        return null;
    }
  }

  static parseCarteDeplacement(jsonObj) {
    const valeur_deplacement =
      jsonObj.type_deplacement === "absolu"
        ? jsonObj.index_case
        : jsonObj.nombreDePas;

    return new CarteAction(jsonObj.titre, jsonObj.description, [
      new DeplacementEffet(
        jsonObj.type_deplacement,
        valeur_deplacement,
        jsonObj.bonusPassage,
      ),
    ]);
  }
