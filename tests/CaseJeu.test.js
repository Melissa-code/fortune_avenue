...
describe("CasePropriete", () => {
  ...
  test("garde seulement la proposition disponible", () => {
    ...
    jest
      .spyOn(Proposition, "getListePropositions")
  ...
  test("arriver() sur une case propriete libre propose d'acheter", () => {
    ...
    jest
      .spyOn(caseJeu, "filtrerPropositionsValables")
      .mockReturnValue([{ type: "acheter" }]);

    caseJeu.arriver(joueur, jeu);

    expect(caseJeu.filtrerPropositionsValables).toHaveBeenCalledWith(
      joueur,
      jeu,
    );
  });
...
describe("CaseAction", () => {
  ...
  test("ajouterEffet() ajoute un effet à la liste", () => {
    const caseAction = new CaseAction("Chance 3", TypesCases.CHANCE);
    const effet = { appliquer: jest.fn() };

    caseAction.ajouterEffet(effet);

    expect(caseAction.effets).toHaveLength(1);
    expect(caseAction.effets[0]).toBe(effet);
  });

  test("arriver() applique tous les effets et retourne les messages", () => {
    const caseAction = new CaseAction("Chance 1", TypesCases.CHANCE);
    const effet = {
      appliquer: jest.fn(() => ["message 1", "message 2"]),
    };
    caseAction.ajouterEffet(effet);

    const joueur = { nom: "Aline" };
    const jeu = { banque: { nom: "Banque" } };

    const resultat = caseAction.arriver(joueur, jeu);

    expect(effet.appliquer).toHaveBeenCalledWith(joueur, jeu, jeu.banque);
    expect(resultat).toEqual(["message 1", "message 2"]);
  });

  test("arriver() retourne [] si aucun effet n est associé", () => {
    const caseAction = new CaseAction("Chance 2", TypesCases.CHANCE);

    const resultat = caseAction.arriver({ nom: "Elie" }, { banque: {} });

    expect(resultat).toEqual([]);
  });

  test('cas "Fonds communs 13" : filtre les propositions fonds communs', () => {
    const propositionA = { titre: "A" };
    const propositionB = { titre: "B" };

    jest
      .spyOn(Proposition, "getListePropositionsFondsCommuns")
      .mockReturnValue([propositionA, propositionB]);

    const caseAction = new CaseAction("Fonds communs 13", TypesCases.FONDS_COMMUNS);
    const joueur = { nom: "Michel" };
    const jeu = { etat: EtatsJeu.EN_COURS };

    const resultat = caseAction.arriver(joueur, jeu);

    expect(resultat).toEqual([propositionA, propositionB]);
    expect(jeu.listePropositions).toEqual([propositionA, propositionB]);
