...
describe('CarteAction', () => {
    ...
    test('executer() retourne les messages des effets appliqués', () => {
        const effetMock = {
            appliquer: () => ['Message test']
        };
        const effet1 = { appliquer: () => ['Message effet 1'] };
        const effet2 = { appliquer: () => ['Message effet 2'] };
        const carteAction = new CarteAction('Test Titre Carte Action', 'Description', [effet1, effet2]);
        expect(carteAction.executer()).toEqual(['Message effet 1', 'Message effet 2']);
    });

    // Test de la méthode ajouterEffet() de CarteAction
    test('ajouterEffet() ajoute un effet à la liste des effets', () => {
        const carteAction = new CarteAction('Test Titre Carte Action', 'Description');
